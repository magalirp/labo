...
ksemilla_azar  <- c(562399) # semilla 2
...
hs  <- makeParamSet(
          ...
          makeIntegerParam("maxdepth" , lower=   5L,  upper=   13L),  #la letra L al final significa ENTERO
...
ArbolesCrossValidation  <- function( semilla, data, param, qfolds, pagrupa )
{
  ...
  ganancias  <- mcmapply( ArbolSimple, 
                          ...
                          SIMPLIFY= FALSE,
                          mc.cores= 1)   #debe ir 1 si es Windows (decía 5)
...
dataset  <- fread("./datasets/competencia1_2022_fe.csv")   #donde entreno
...
dir.create( "./exp/HT7110/", showWarnings = FALSE )
setwd("./exp/HT7110/")   #Establezco el Working Directory DEL EXPERIMENTO

#defino los archivos donde guardo los resultados de la Bayesian Optimization
archivo_log  <- "HT7110.txt"
archivo_BO   <- "HT7110.RDATA"
