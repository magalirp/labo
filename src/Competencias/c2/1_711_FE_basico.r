...
dataset$ctrx_quarter_normalizado <- as.numeric(dataset$ctrx_quarter_normalizado)
...
dataset$mlimitecompra_total <- rowSums (dataset [, c (121,143)], na.rm = TRUE )
sum(is.na(dataset$mlimitecompra_total)) #[1] 0
...
rankeadas <- rankear(seleccionadas) 


no_seleccionadas <- dataset %>% 
...
dataset_rank <- no_seleccionadas %>% 
  left_join(., rankeadas, by = "numero_de_cliente")  

 
names(no_seleccionadas)
names(rankeadas)
----------------------------------------------------------------------------------------------------------------------------------------------------------
