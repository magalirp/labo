...
PARAM  <- list()
PARAM$experimento <- "FE4003"

PARAM$exp_input  <- "DR4001"

PARAM$lag1  <- TRUE
PARAM$lag2  <- TRUE
PARAM$Tendencias  <- TRUE #VENTANA 6 MESES
...
AgregaVarRandomForest  <- function( num.trees, max.depth, min.node.size, mtry)
{
  ...
  dataset_rf[ , entrenamiento := as.integer( foto_mes>= 201907 &  foto_mes<= 201912 & ( clase01==1 | azar < 0.10 )) ]
